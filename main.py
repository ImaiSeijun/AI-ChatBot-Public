...
f = open('setting_sentences.txt', 'r')

setting_sentences = f.read()
...
def chat_with_gemini(user_input: str) -> str:
        try:
            ...
            text = response.text.strip() if response.text else "（返答が空でした）"
            return text if text else "（返答が空でした）"
        except Exception as e:
            print(f"Geminiエラー: {e}")
            return "Geminiとの会話中にエラーが発生しました。"
